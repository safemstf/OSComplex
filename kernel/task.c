...
task_t *task_create_user(const char *name, void *elf_data, uint32_t priority)
{
    ...
    task->user_esp = USER_STACK_TOP - 16; // ← Add the -16
    ...
    terminal_writestring("[TASK_CREATE_USER]   Kernel ESP=0x");
    terminal_write_hex(task->context.esp);
    terminal_writestring("\n");
    terminal_writestring("[TASK_CREATE_USER]   Entry Point=0x");
    terminal_write_hex(task->entry_point);
    terminal_writestring(" (on stack)\n");
...
void task_switch(task_t *new_task)
{
    ...
    /* CRITICAL: Update TSS kernel stack for interrupts in user mode */
    extern void tss_set_kernel_stack(uint32_t stack);
    tss_set_kernel_stack(new_task->kernel_stack + 4096);

    /* For user mode tasks running for the FIRST time */
    if (new_task->ring == 3 && new_task->context.eip == 0)
    {
        terminal_writestring("[SWITCH] First-time user mode entry!\n");
        terminal_writestring("[SWITCH]   Entry: 0x");
        terminal_write_hex(new_task->entry_point);
        terminal_writestring("\n");

        /* Set user segments and jump via iret */
        __asm__ volatile(
            "mov $0x23, %%ax\n"
            "mov %%ax, %%ds\n"
            "mov %%ax, %%es\n"
